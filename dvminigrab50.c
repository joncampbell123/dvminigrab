...
/* these too */
int			capture_buffer_frame_size = 12000*10*2;
...
void modify_header_enable_audio(unsigned char *frame,int sz) {
	...
	for (;frame < fence;frame += 150*80*2) {
...
int open_capture() {
	...
	capture_pos -= capture_pos % (12000 * 10 * 2);
...
long long holding_limit = ( 12000 * 10 * 2 ) * 30 * 60;	// 1 min
...
int empty_capture_buffer(int max) {
	...
	while (capture_buffer_i != capture_buffer_o && wrote < max) {
		if ((capture_pos % (12000 * 10 * 2)) != 0) {
			printf("Warning: capture pos is not frame aligned (%lu)\n",(unsigned long)(capture_pos % (12000 * 10 * 2)));
			capture_pos += 12000 * 10 * 2;
			capture_pos -= capture_pos % (12000 * 10 * 2);
		}
...
again:
		...
		/* and log frame times */
		{
			...
			for (i=capi,o=0;c-- > 0;capi++,o++) {
				...
				sprintf(tmp,"@%.3f: DV=%lld wr=%d/%d idx=%d/%d\n",capture_buffer_time[capi],(int64_t)start+(o*12000*10LL*2),
...
unsigned char dv25_assembly[12000*12*2]; /* enough even for the PAL version */
...
void emit_assembled_dv25() {
	store_output(dv25_assembly,12000*10*2);
}
...
int start_iso_recv_dv25() {
	...
	if (raw1394_iso_recv_init(raw1394, iso_handler_dv25, (1500/6) * 3 * 30, 8+(80*6*2), channel, recv_mode, 64) < 0) { /* up to 3 seconds */
...
int main(int argc,char **argv) {
	...
	dv25_discard = 1;
	init_dv25_assembly();
	if (init_capture_buffer(30*4,12000*10*2)) { /* 4 second video buffer of NTSC DV */
